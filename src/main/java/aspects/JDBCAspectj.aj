...
aspect JDBCAspect {
	...
	protected pointcut methodTraced(): call(!static * *.GestorBD.*(..))   && !within(JDBCAspect) && !execution(* Object.*(..));
	...
	after(String sql): misuse_of_executeQuery(sql) {
		_logger.log(Level.INFO,
				indent + ANSI_YELLOW+"\t Revisa el codigo! Estas invocando al metodo executeQuery pasnddole como parametro la instruccion SQL: "
	...
	after() throwing  (Throwable ex):exceptionTraced(){
		...
		if (lastLoggedException.get() != ex) {
			...
			dataMap.put(thisEnclosingJoinPointStaticPart.getSignature().getName(), new int[13]);
		}
	...
	protected void afterLog(String indent, JoinPoint joinPoint) {
		...
		if (aux[6] != 0 && aux[7] == 0) {
			_logger.log(Level.INFO, indent
					+ ANSI_YELLOW+"\t Revisa el codigo! Tu metodo JDBC crea una conexion con la BD pero, tras terminar de trabajar, no liberas en recurso.");
			resultCSinC=0;

		} else if (aux[6] != 0 && aux[7] != 0) {
			_logger.log(Level.INFO,
					indent + ANSI_GREEN+"\t Bien hecho! Tu metodo JDBC crea una conexion con la BD y ademas, tras terminar de trabajar, liberas en recurso.");
			resultCSinC=1;
		...
		if (aux[11] != aux[12]) {
			_logger.log(Level.INFO, indent
					+ ANSI_YELLOW+"\t Revisa el codigo! Invocas a metodos del tipo getXXX siendo XXX tipos de datos primitivos, pero no compruebas la posibilidad de obtener valores nulos (wasNull).");
			resultNotWasNull=0;
		}
		_logger.log(Level.INFO, indent + "\t******************************* Fin del metodo: " + name + "*****************************");
		_logger.log(Level.WARN, "Pr4;"+name+";"+resultPS+";"+resultS+";"+resultRS+";"+resultCSinC+";"+resultCCC+";"+resultConAUSinC+";"+resultSinAUConC+";"+resultConAUConC+";"+resultGetConNombres+";"+resultNotWasNull);
	}
